    export const jQuery :any= undefined;
    export const Popper: any = undefined;
	...
	export const UDAPluginSDK:any={
		...
		inArray:function(value:any, object:any){
		...
		loadScript: function(url:string) {
		...
		loadCssScript: function(url:string) {
		...
		addvoicesearchmodal:function(addnisticon=true){
			...
			if(this.multilingual.enabled) {
				this.bcplang.forEach((langcode:any) => {
					if (langcode.length > 2) {
						langcode.forEach((sublang:any, sublangindex:number) => {
							if (sublangindex !== 0) {
		...
		getAlertHtml: function(content:any, addContinueBtn=true, continueText='Continue with errors', exitText='Exit UDAN'){
			let footerHtml = '';
			if(addContinueBtn){
		...
		showAlert: function(storageName:any, content='', addContinueBtn=false, continueText='Continue', exitText='Close'){
		...
		alertDecline: function(storageName:any){
		...
		alertAcceptance: function(storageName:any){
		...
		openmodal:function(focus=false){
        	if(this.sessiondata.authenticated) {
				...
				if (bodychildren.length > 0) {
					bodychildren.forEach(function (childnode:any, childnodeindex:number) {
		...
		indexdom: function( node:any, ret=false, parentnode="", textlabel="", hasparentnodeclick=false, parentclicknode= null ) {
		...
		checkCssClassNames:function(node:any){
		...
		addClickToNode:function(node:any, confirmdialog=false){
        	try{
		...
		performclickaction:function(selectednode:any,navcookiedata:any){
			const matchNodes :any= [];
			...
			if(matchNodes.length === 1){
				...
				matchNodes.forEach(function (matchNode:any, matchnodeindex:number) {
		...
		comparenodes:function(comparenode:any, originalnode:any, isPersonalNode=false, match={count:0, matched:0, unmatched:[], innerTextFlag: false, innerChildNodes: 0}){
			// sum the childnodes
		...
		processDistanceOfNodes: function(matchingnodes:any, selectedNode:any) {
		 ...
		 * @param2: comparing node
 		 */
		getDistance: function (node1:any, node2:any) {
			let dist;
		...
		createstoragedata:function(key:string,value:string){
			try {
		...
		deleteSequence:function(data:any){
			let sequencename = '';
			try{
				let sequencenamesArray = JSON.parse(data.name)
				sequencename = sequencenamesArray[0];
		...
		toggleautoplay:function(navcookiedata:any){
			if(navcookiedata.autoplay){
		...
		updatenavcookiedata:function(navcookiedata:any,selectednodeid:any){
