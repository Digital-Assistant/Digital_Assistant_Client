...
function UDACheckStoredSessionData(storedsessiondata, getsession=true){
	...
	if(storedsessiondata.hasOwnProperty("sessionkey") && storedsessiondata.sessionkey){
		if(typeof UDAUserAuthData!='undefined' && UDAUserAuthData.id!=null && (storedsessiondata.authdata.hasOwnProperty('id') && storedsessiondata.authdata.id === UDAUserAuthData.id)){
			UDASessionData=storedsessiondata;
			UDASendSessionData();
			clearInterval(UDAIntervalid);
		} else if(getsession){
	...
	xhr.onload = function(event){
		if(xhr.status === 200){
			if(UDASessionData.sessionkey !== null) {
				...
				UDAGetSessionKey();
				// UDASendSessionData();
				setTimeout(function (){UDASendAuthData();},2000);
			}
		}
	};
...
function UDACheckSessionStorage(){
	const storedsessiondata = window.localStorage.getItem(UDASessionName);
	if(storedsessiondata != null){
		UDACheckStoredSessionData(storedsessiondata, false);
	}
}

UDAIntervalid = setInterval(() => {
	UDACheckSessionStorage();
}, 500);
