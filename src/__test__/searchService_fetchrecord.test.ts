...
describe("fetchRecord", () => {
  ...
  // test that fetchRecord throws an error if required parameters are missing
  /**
   * Verifies that the `fetchRecord` function throws an error if required parameters are missing.
   *
   * This test case checks that when the `fetchRecord` function is called without any parameters, or with only the `id` or `domain` parameter, it throws an error with the message "Required parameters are missing".
   */
  it("should throw an error if required parameters are missing", async () => {
    // no params
    await expect(fetchRecord()).rejects.toThrow(
      "Required parameters are missing"
    );

    // only id param
    await expect(fetchRecord({ id: "123" })).rejects.toThrow(
      "Required parameters are missing"
    );

    // only domain param
    await expect(fetchRecord({ domain: "example.com" })).rejects.toThrow(
      "Required parameters are missing"
    );
  });
  ...
  it("should handle request with valid id, domain, and additionalParams", async () => {
    ...
    expect(REST.apiCal).toHaveBeenCalledWith({
      // verify that the request was processed with the correct parameters
      // including the id, domain, and additionalParams
      url: `${
        ENDPOINT.fetchRecord
      }/withPermissions/456?domain=example.org&additionalParams=${encodeURIComponent(
        JSON.stringify(request.additionalParams)
  ...
  it("should handle request with empty additionalParams", async () => {
    ...
    expect(REST.apiCal).toHaveBeenCalledWith({
      url: `${
        ENDPOINT.fetchRecord
      }/withPermissions/abc?domain=example.com&additionalParams=${encodeURIComponent(
        JSON.stringify({})
  ...
  it("should handle request with additionalParams containing boolean values", async () => {
    ...
    expect(REST.apiCal).toHaveBeenCalledWith({
      ...
      url: `${
        ENDPOINT.fetchRecord
      }/456?domain=example.org&additionalParams=${encodeURIComponent(
        JSON.stringify(request.additionalParams)
      )}`,
      // verify that the method is GET
      method: "GET",
  ...
  it("should fetch record with additionalParams containing nested objects", async () => {
    ...
    // verify that the API call was made with the correct parameters
    expect(REST.apiCal).toHaveBeenCalledWith({
      url: `/search/withPermissions/789?domain=example.net&additionalParams=${encodeURIComponent(
        JSON.stringify(request.additionalParams)
  ...
  it("should fetch record with empty additionalParams", async () => {
    ...
    // Verify that the API call was made with the correct parameters
    expect(REST.apiCal).toHaveBeenCalledWith({
      // URL with empty additionalParams
      url: `${
        ENDPOINT.fetchRecord
      }/withPermissions/456?domain=example.org&additionalParams=${encodeURIComponent(
        // Empty additionalParams should be sent as an empty string
        JSON.stringify(request.additionalParams) === "{}"
          ? ""
          : JSON.stringify(request.additionalParams)
      )}&userSessionId=user123`,
