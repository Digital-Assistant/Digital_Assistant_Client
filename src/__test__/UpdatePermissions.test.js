...
describe("RecordSequenceDetails", () => {
  test("addSkipClass should return correct class", () => {
    const item = {
      status: "completed",
      objectdata: { meta: { skipDuringPlay: true } },
    };
    expect(RecordSequenceDetails.addSkipClass(item)).toBe(
      "skipped uda_exclude"
    );

    const item2 = {
      status: "in progress",
      objectdata: { meta: { skipDuringPlay: false } },
    };
    expect(RecordSequenceDetails.addSkipClass(item2)).toBe(
      "in progress uda_exclude"
    );
  });
