...
describe("searchService", () => {
  describe("fetchSpecialNodes", () => {
    ...
    it("should handle errors when fetching special nodes", async () => {
      ...
      // Assert
      // Assert that the fetchSpecialNodes function is called and the UDAErrorLogger.error function is called with the expected arguments
      expect(fetchSpecialNodes).toHaveBeenCalled();
      expect(UDAErrorLogger.error).toHaveBeenCalledWith(
        "Error fetching special nodes",
        mockError
      );
    });
    ...
    it("should handle errors when fetching special nodes", async () => {
      ...
      expect(fetchSpecialNodes).toHaveBeenCalled();
      // Assert that the UDAErrorLogger.error function is called with the expected arguments
      expect(UDAErrorLogger.error).toHaveBeenCalledWith(
        "Error fetching special nodes",
        mockError
      );
    ...
    it("should fetch special nodes with additional parameters", async () => {
      ...
      // Assert
      // Assert that the REST.processArgs function was called with the expected arguments
      expect(REST.processArgs).toHaveBeenCalledWith(
        ENDPOINT.SpecialNodes,
        request
      );

      // Assert that the REST.apiCal function was called with the expected arguments
    ...
    it("should fetch special nodes without additional parameters", async () => {
      ...
      // Assert
      // Assert that REST.processArgs was called with the expected arguments
      expect(REST.processArgs).toHaveBeenCalledWith(
        ENDPOINT.SpecialNodes,
        undefined
      );
      // Assert that REST.apiCal was called with the expected arguments
      expect(REST.apiCal).toHaveBeenCalledWith({
        url: "mock-url",
        method: "GET",
      });
    ...
    it("should handle errors when fetching special nodes with additional parameters", async () => {
      ...
      // Act: call the fetchSpecialNodes function with the mock request object
      await expect(fetchSpecialNodes(request)).rejects.toThrow(mockError);

      // Assert: check that the REST.processArgs function was called with the expected arguments
      expect(REST.processArgs).toHaveBeenCalledWith(
        ENDPOINT.SpecialNodes,
        request
      );
      // Assert: check that the REST.apiCal function was called with the expected arguments
      expect(REST.apiCal).toHaveBeenCalledWith({
        url: "mock-url",
        method: "GET",
      });
      // Assert: check that the UDAErrorLogger.error function was called with the expected arguments
      expect(UDAErrorLogger.error).toHaveBeenCalledWith(
        "Error fetching special nodes",
        mockError
      );
    ...
    it("should handle errors when fetching special nodes without additional parameters", async () => {
      ...
      // Assert that the UDAErrorLogger.error function was called with the expected arguments
      expect(UDAErrorLogger.error).toHaveBeenCalledWith(
        "Error fetching special nodes",
        mockError
      );
    ...
    it("should not call the UDAErrorLogger if the fetch is successful", async () => {
      ...
      // Assert: check that the UDAErrorLogger.error function is not called
      expect(REST.processArgs).toHaveBeenCalledWith(
        ENDPOINT.SpecialNodes,
        undefined
      );
    ...
    it("should return an empty object when no special nodes are found", async () => {
      // Arrange: mock the REST.apiCal function to return an empty object
      const mockData = {};
    ...
    it("should return special nodes with single property", async () => {
      ...
      const mockData = { specialNode1: "value1" };
