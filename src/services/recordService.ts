...
export const saveClickData = async (node: any, text: string, meta: any) => {
  try {
    ...
    // Serialize the object data to a JSON string using TSON.
...
export const recordUserClickData = async (
  clickType = "sequencerecord", // Type of click event to record
  clickedName = "", // Name of the element that was clicked
  recordId: number = 0 // ID of the record to associate with the click
) => {
  try {
    // Construct the payload object by retrieving the user session key and adding the other properties
    const payload = {
      usersessionid: await getSessionKey(), // User session key
      clickedname: clickedName, // Name of the element that was clicked
      clicktype: clickType, // Type of click event to record
      recordid: recordId, // ID of the record to associate with the click
    };
    // Call the userClick function with the constructed payload and return the result
